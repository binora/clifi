...
- [ ] Safe signing pipeline: intent struct, policy checks (spend limits/allowlists), simulation via `eth_call`, explicit confirmation gates, nonce management, gas estimation/fee display. *Next up*.
- [ ] Implement send/approve: native sends, ERC20 sends, ERC20 approvals; wire through tools + CLI with prompts (after pipeline scaffolding).
